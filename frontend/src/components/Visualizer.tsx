...
const Visualizer = ({ microphone }: { microphone: MediaRecorder }) => {
  ...
  useEffect(() => {
    if (!microphone || !microphone.stream) {
      console.error("Visualizer: Microphone or stream is not available.");
      return;
    }
    ...
    return () => {
      ...
      if (audioContext.state !== 'closed') {
        // Don't close the context if it was created outside, but maybe suspend?
        // Or perhaps let the caller manage the context lifecycle.
        // For simplicity here, we just disconnect.
      }
    };
